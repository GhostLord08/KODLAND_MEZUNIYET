...
async def on_message(message):
    ...
    if message.content.startswith('HELLO LORD'):
        ...
        for attachment in message.attachments:
            ...
            print(img_class[0])
            if img_class[0].strip() == 'ARDUINO UNO':
                await message.channel.send("This is an Arduino UNO board.")
            elif img_class[1].strip() == 'ARDUINO NANO':
                await message.channel.send("This is an Arduino Nano board.")
            elif img_class[2].strip() == 'ARDUINO MEGA':
                await message.channel.send("This is an Arduino Mega board.")
            if not message.atechment:
                await message.channel.send("I have no idea what this is.")

    else:
        await message.atechment(message.content)
        



client.run("ODI4MTg2MTA4NzU1MDUwNTI2.GoMWyC.gDIhaeTtxNhwFdKZ6KVjfYvFy1dTEl215ggqvY")